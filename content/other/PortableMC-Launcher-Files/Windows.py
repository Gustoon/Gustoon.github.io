...
if platform.system() != "Windows" and platform.system() != "Linux" and platform.system() != "Darwin":
    print("This Launcher is only supported on Windows.")
    sys.exit(1)
...
def Login():
    ...
    canvas = tkinter.Canvas(master=root, highlightthickness=0)
...
def MainWindow():
    ...
    def OpenLog():
        if not os.path.exists("./Personal/logs/"):
            ShowMSBox(master=SettingsFrame, Title="Log folder error", text="There is no folder for logs. Maybe because you didn't\n start the game?", button1="OK", IconStyle="Windows 11", Icon='Error')
        else :
        ...
        if ServerAdress != None:
            if os.path.getsize("./Saver/connection.txt") == 0:
                with open("./Saver/username.txt", "r+") as f:
                    ...
                    f.close()
                os.system('portablemc --main-dir ./MC/ --work-dir ./Personal/ start -s "' + ServerAdress + '" -p "' + ServerPort + '" --jvm-args="-Xmx' + str(int(MCRam)) + "M -Xms" + str(int(MCRam)) + 'M" --username ' + MCUsername + " " + version)
            else:
                with open("./Saver/connection.txt", "r+") as f:
                    MCLogin = f.readline()
                    f.close()
                os.system('portablemc --main-dir ./MC/ --work-dir ./Personal/ start -s "' + ServerAdress + '" -p "' + ServerPort + '" --jvm-args="-Xmx' + str(int(MCRam)) + 'M" -l ' + MCLogin + " " + version)
            ...
            if os.path.getsize("./Saver/connection.txt") == 0:
                with open("./Saver/username.txt", "r+") as f:
                    MCUsername = f.readline()
                    f.close()
                os.system('portablemc --main-dir ./MC/ --work-dir ./Personal/ start --jvm-args="-Xmx' + str(int(MCRam)) + "M -Xms" + str(int(MCRam)) + 'M" --username ' + MCUsername + " " + version)
            else:
                with open("./Saver/connection.txt", "r+") as f:
                    MCLogin = f.readline()
                    f.close()
                os.system('portablemc --main-dir ./MC/ --work-dir ./Personal/ start --jvm-args="-Xmx' + str(int(MCRam)) + "M -Xms" + str(int(MCRam)) + 'M" -l ' + MCLogin + " " + version)
            PlaySection.configure(state='normal')
            SettingsSection.configure(state='normal')
            PlayButton.configure(state='normal')
            Disconnect_button.configure(state='normal')
